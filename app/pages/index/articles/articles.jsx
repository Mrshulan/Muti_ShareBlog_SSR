...
const NoData = ({ keyword, categories}) => (
  <React.Fragment>
    没有关于<span>{keyword ? keyword : categories ? categories : "该技术"}</span>的文章！ 
    <br/>
    期待你的创作~
  </React.Fragment>
)
  ...
  const list = articlesList && articlesList.map((item) => {
    return (
      <CSSTransition
        in={isArticlesLoaded}
        key={item._id}
        stylenames="article-item"
        timeout={500}
      >
        <li key={item._id} styleName='article-item have-img'>
          <a styleName="wrap-img" href={`/article/${item._id}`} >
            <img  data-src={item.author.avatar} src={(isPro ? 'http://mrshulan.xin' : 'http://127.0.0.1:6001') + item.author.avatar} alt="摘要图片" />
          </a>
          <div styleName="content">
            ...
            <div styleName="meta">
              ...
              <span >{timestampToTime(item.created)}</span>
            </div>
          </div>
        </li>
      </CSSTransition>
...
//     return (
//       <div styleName="left" ref={(left) => { this.left = left}}>
//         <ul styleName="article-list">
//           <TransitionGroup>
//             {list}
//           </TransitionGroup>
